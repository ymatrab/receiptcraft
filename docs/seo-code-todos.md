...
- **NON-ISSUE — 404 "duplicate robots meta".** Investigated + verified against the live 404: this Next build **auto-injects its own `noindex`** on every 404 **and** still inherits the layout's sitewide `index, follow` unless overridden. So `not-found.tsx` must keep an explicit `robots: noindex` (incl. `googleBot`) to suppress the inherited `index, follow`. That leaves two `<meta name="robots">` tags (framework's + ours) — **both noindex, so harmless**, and a 404 HTTP status keeps the page out of the index regardless. The duplicate is **unavoidable and not worth chasing** — an attempt to remove our explicit robots caused `index, follow` to leak and was reverted. Do not touch.
