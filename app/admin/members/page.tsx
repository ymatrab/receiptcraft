...
export default async function AdminMembers() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink-soft">{members.length} most recent</p>
              ...
              return (
                <tr key={m.id}>
                  <td className="px-4 py-3 font-medium text-ink">
                    {m.email ?? "—"}
                    {m.is_admin && (
                      <span className="ml-2 rounded bg-ink px-1.5 py-0.5 text-[10px] font-semibold text-white">
                        ADMIN
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3 text-ink-soft">{m.full_name ?? "—"}</td>
                  <td className="px-4 py-3">
                    {m.provider ? (
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                        pro ? "bg-emerald-100 text-emerald-700" : "bg-rule/45 text-ink-soft"
                      }`}
                    >
                      {pro ? m.sub?.plan ?? "pro" : "free"}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-medium text-ink-soft tabular-nums">
                    {m.downloads}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {pro ? (
                      <form action={revokePro}>
                        ...
                        <button className="rounded-[3px] border border-red-200 px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50">
                          Revoke
                        </button>
                      </form>
                    ) : (
                      <div className="flex gap-1.5">
                        <form action={grantPro}>
                          ...
                          <button className="rounded-[3px] border border-emerald-200 px-2 py-1 text-xs font-medium text-emerald-700 hover:bg-emerald-50">
                            Grant 7d
                          </button>
                        </form>
                        <form action={grantPro}>
